...
<<'###'
For some reason, kpartx cannot mount the loopback device after a successful previous mount, probably a bug in kernel.
Also sleep and retry don't work unless the whole script is run again. Return 9 so the calling script would retry.
###
LOOP_DEVICE=$(echo $DEVICE|cut -f4 -d '/')
if [[ $LOOP_DEVICE != loop* ]]; then
  exit 9
fi
...
echo "----Adding linpeas.sh to image----"
mkdir "${IMAGE_DIR}/analysis"
cp "${SCRIPT_DIR}/linpeas.sh" "${IMAGE_DIR}/analysis/linpeas.sh"

echo "----Modify inittab----"
# Getting root shell as well as running linpeas however
#sed -i 's/^tty.*/::respawn:-\/bin\/sh\n\::wait:\/analysis\/linpeas.sh/' "${IMAGE_DIR}/etc/inittab"
# For automation, we just need to add the entry to run the script
echo "::wait:/analysis/linpeas.sh" >> "${IMAGE_DIR}/etc/inittab"
sleep 1
